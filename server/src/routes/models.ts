...
import { getKeyCount, OPENAI_PREFIX, OPENROUTER_PREFIX } from "../services/keyRotation.js";

const router = Router();

// GET /api/models — list available models with key availability info
router.get("/", (_req: Request, res: Response): void => {
  const openaiKeys = getKeyCount(OPENAI_PREFIX);
  const openrouterKeys = getKeyCount(OPENROUTER_PREFIX);

  const models = AVAILABLE_MODELS.map((m) => ({
    ...m,
    available:
      m.provider === "openai" ? openaiKeys > 0 : openrouterKeys > 0,
  }));
